...
public class PropellerSDK : MonoBehaviour
{
	...
	public static bool SetUserConditions (Dictionary<string, string> conditions)
	{
		...
		if (!Application.isEditor) {
			...
			JSONClass conditionsJSON = toJSONClass (conditionsToJSON);
	...
	public static bool SyncUserValues ()
	{
		...
		if (!Application.isEditor) {
#if UNITY_IPHONE
			succeeded = PropellerImports.iOSSyncUserValues();
#elif UNITY_ANDROID
	...
	{
		Debug.Log ("PropellerOnUserValues");

		// message must be defined or else its an error
		if (message == null) {
			Debug.Log ("PropellerOnUserValues - null message");
			return;
		}

		// can have no user values in the case where the client
		// was unable to contact the dynamics server and there
		// are no cached values

		Debug.Log ("PropellerOnUserValues - " + message);
		...
		if (message.Length > 0) {
			...
			if ((resultsArray.Length % 2) == 1) {
				Debug.LogError ("PropellerOnUserValues - Invalid response from PropellerUnitySDK: " + message);
				return;
			}
		}
